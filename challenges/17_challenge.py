...
def is_activated(ary, x, y, z):
    """Return the next state of a cube at (x,y,z)."""
    inital_val = ary[x, y, z]
    cube = ary[(x - 1) : (x + 2), (y - 1) : (y + 2), (z - 1) : (z + 2)]
    cube[1, 1, 1] = 0
    total = np.sum(cube)
...
for i in range(n_cycles):
    ...
    for x, y, z in product(*index_ranges):
        conway_cubes[x, y, z] = is_activated(padded_cubes.copy(), x + 1, y + 1, z + 1)
